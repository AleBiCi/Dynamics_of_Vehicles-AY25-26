...
function [Gxa, Gyk, SVyk] = MF96_FXFYCOMB_coeffs(kappa, alpha, gamma__w, Fz, tyre_data)
 ...
 % main code
  ...
  SHyk = rHy1;
  % redefine mu__y
  mu__y = (dfz .* pDy2 + pDy1) .* (-pDy3 .* gamma__w .* LGAMMAY .^ 2 + 1) .* LMUY;

  DVyk = mu__y .* Fz .* (rVy2 .* dfz + rVy3 .* gamma__w + rVy1) .* (rVy4 .^ 2 .* alpha .^ 2 + 1) .^ (-0.1e1 ./ 0.2e1);
